<template>
  <section class="export-panel">
    ...
    <button @click="downloadSummary">📤 Export Et</button>
  </section>
</template>
...
onMounted(() => {
  ...
  new Chart(trendChart.value, {
    type: 'line',
    data: {
      labels: sprint.modules.map((_, i) => `Sprint ${i + 1}`),
      datasets: [{
        label: 'Modül Aktivitesi',
        data: sprint.modules.map(m => m.progress),
        borderColor: '#4A90E2',
...
const generatePDF = async () => {
  ...
  reportUrl.value = (await response.json()).url
}
